...
const createInventoryController = async (req, res) => {
  try {
    ...
    // Set hospital ID for orgnaisation or hospital roles
    if (role === "orgnaisation" || role === "hospital") {
    ...
    // Ensure orgnaisation ID is set
    req.body.organisation = user._id; // Assuming the user is associated with the orgnaisation
...
const getRecentInventoryController = async (req, res) => {
  try {
    // Fetch all inventory records regardless of the user's orgnaisation
    const inventory = await inventoryModel
      .find({})
      .limit(1000)
      .sort({ createdAt: -1 });
